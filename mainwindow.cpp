...
void MainWindow::convertImage()
{
    ...
    if (!fileName.isEmpty()) {
            ...
            if (extension == "bmp") 
            {
                SDL_SaveBMP(SDLContainer::surface.ptr.get(), fileName.toStdString().c_str());
                std::cout << "Converting to BMP format" << std::endl;
            } 
            else if (extension == "jpg" || extension == "jpeg") {
                IMG_SaveJPG(SDLContainer::surface.ptr.get(), fileName.toStdString().c_str(), 100);
                std::cout << "Converting to JPEG format" << std::endl;
            } 
            else if (extension == "png") {
                IMG_SavePNG(SDLContainer::surface.ptr.get(), fileName.toStdString().c_str());
